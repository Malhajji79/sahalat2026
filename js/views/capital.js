function capitalMovementsView(){
  ...
  const live = state.currentUser.role==='أدمن'
    ...
    : liveRows.filter(t=>t.from===state.currentUser.username || t.to===state.currentUser.username || (state.currentUser.role==='مدير مشروع' && t.createdById===state.currentUser.id)).slice().reverse();

  const historical = ['أدمن','مدير مشروع'].includes(state.currentUser.role)
    ...
    <details class="card" id="capitalHistorySection" style="margin-top:20px"><summary style="cursor:pointer;font-weight:800">${tx('السجل التاريخي لرأس المال — إظهار الكل / إخفاء','Historical Capital Log — Show All / Hide')}</summary>
      ...
      <div class="table-wrap" style="margin-top:14px">
        <table>
          ...
          <tbody>
            ${historical.length ? htmlJoin(historical.map(t=>html`<tr>
              <td>${t.seq}</td>
              ...
              <td>${t.name||'—'}</td>
            </tr>`)) : html`<tr><td colspan="7" class="muted">${tx('لا توجد سجلات تاريخية مرتبطة بهذا الحساب.','No historical records are linked to this account.')}</td></tr>`}
          </tbody>
        </table>
      </div>
    </details>`;

  const liveTable=html`
      <div class="card" style="margin-top:18px">
        <h3 class="section-title">${tx('حركات رأس المال الجديدة','New Capital Movements')}</h3>
        ${live.length>5?html`<button class="btn btn-secondary" id="showAllCapitalMovements" type="button" aria-expanded="false" aria-controls="newCapitalMovementsBody">${tx('إظهار الجميع','Show All')}</button>`:''}
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="newCapitalMovementsBody">${live.length?htmlJoin(live.map((t,index)=>html`<tr ${index>=5?'hidden':''}>
              ...
              <td>${t.createdBy||'—'}</td>
            </tr>`)):html`<tr><td colspan="8" class="muted">${tx('لا توجد حركات جديدة بعد نقطة الانتقال.','No new movements after the transition point.')}</td></tr>`}</tbody>
          </table>
        </div>
      </div>
`;
...
function capitalTransferView(){
  if(state.currentUser.role==='أدمن'){
    return html`<div class="card">${tx('الأدمن لا يستخدم تعديل رأس المال من الصفحة الرئيسية للمستخدم.','Admin does not use capital editing from the user home page.')}</div>`;
  }
    ...
    <div class="grid" style="grid-template-columns:1fr 1fr;gap:18px">
      <div class="panel">
        ...
        ${state.currentUser.role==='مستخدم'
          ? html`<div class="field"><label>${tx('المستخدم','User')}</label><input value="${state.currentUser.username}" disabled></div>`
          : html`<div class="field">
              <label>${tx('المستخدم المحدد','Selected User')}</label>
              <select id="capitalSelectedUser">
                ${htmlJoin(users.map(u=>html`<option value="${u.username}" ${u.username===state.accountUser?'selected':''}>${u.username}</option>`))}
              </select>
            </div>`}
          ...
          <table>
            ...
            <tbody>
              ${(state.capitalTransfers||[]).filter(t=>t.from===state.currentUser.username || t.to===state.currentUser.username).length
                ? htmlJoin((state.capitalTransfers||[])
                ...
                <tr>
                  ...
                  <td>
                    ${(canEditCapitalTransfer(t) || canDeleteCapitalTransfer(t))
                      ? `${canEditCapitalTransfer(t)?html`<button class="btn btn-secondary editCapitalTransferBtn" data-id="${t.id}">${tx('تعديل','Edit')}</button>`:''}
                         ${canDeleteCapitalTransfer(t)?html`<button class="btn btn-danger deleteCapitalTransferBtn" data-id="${t.id}">${tx('إلغاء الحركة','Cancel Movement')}</button>`:''}`
                      : ''}
                  </td>
                </tr>
              `)) : html`<tr><td colspan="7" class="muted">${tx('لا توجد حركات رأس مال مرتبطة بحسابك.','No capital movements are linked to your account.')}</td></tr>`}
            </tbody>
          </table>
        </div>
