function adminView(){
  ...
  ${state.currentUser.role==='أدمن'?html`
  ...
  <div class="table-wrap"><table>
    ...
    <tbody>
      ${users.length?htmlJoin(users.map(u=>html`<tr>
        <td>${u.username}</td>
      ...
      </tr>`)):html`<tr><td colspan="6" class="muted">${tx('لا يوجد مستخدمون.','No users found.')}</td></tr>`}
    </tbody>
  </table></div>
...
function accountsView(){
  ...
  if(!m) return html`<div class="card">${tx('لا توجد بيانات لهذا المستخدم.','No data is available for this user.')}</div>`;
  const canEdit = ['أدمن','مدير مشروع'].includes(state.currentUser.role);

  const analyticsPermissionPanel = (state.currentUser.role==='مدير مشروع') ? html`
    ...
    <div class="toolbar">
      <div class="field" style="min-width:280px;margin:0">
        ...
        <select id="accountUserSelect">
          ${htmlJoin(selectable.map(u=>html`<option value="${u.username}" ${u.username===state.accountUser?'selected':''}>${u.username} — ${state.lang==='en'?({'أدمن':'Admin','مدير مشروع':'Project Manager','مدير المشروع':'Project Manager','مستخدم':'User'}[u.role]||u.role):u.role}</option>`))}
        </select>
      </div>
      ${canEdit?html`<button class="btn btn-primary" id="editAccountValues">${tx('تعديل رأس المال','Edit Capital')}</button>`:''}
    </div>
