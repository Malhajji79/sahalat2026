...
function renderDecisionNotifications(){
  ...
  host.innerHTML=items.length?html`<h3>${tx('إشعارات القرارات الأخيرة','Recent Decision Notifications')}</h3>`+htmlJoin(items.map(n=>{
    const subject=n.kind==='loan'?tx('القرض','Loan'):tx('مراجعة فترة السداد للقرض','Repayment review for loan');
    return html`<div class="decision-notice ${n.rejected?'decision-rejected':'decision-approved'}"><div><strong>${n.rejected?tx('تم رفض','Rejected:'):tx('تمت الموافقة على','Approved:')} ${subject} ${n.loan}</strong>${n.rejected?html`<p>${tx('سبب الرفض','Rejection reason')}: ${n.reason||tx('لم يُسجل سبب للرفض.','No rejection reason was recorded.')}</p>`:''}${n.time?html`<div class="small">${dateInputText(n.time)}</div>`:''}</div><button class="btn btn-secondary" type="button" data-dismiss-decision="${n.key}">${tx('إغلاق الإشعار','Dismiss')}</button></div>`;
  })):'';
