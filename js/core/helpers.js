...
function toArabicDigitsText(value){
  ...
  // Keep Sahalat display dates in Latin digits in both Arabic and English UI.
  // Example: 19-Sep-2026
  return text
    .split(/(\d{2}-[A-Za-z]{3}-\d{4})/g)
    .map(part => /^\d{2}-[A-Za-z]{3}-\d{4}$/.test(part)
      ? part
      : part.replace(/[0-9]/g,d=>'٠١٢٣٤٥٦٧٨٩'[Number(d)]))
    .join('');
}
