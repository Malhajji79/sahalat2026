...
function toArabicDigitsText(value){
  ...
  const dates=[];
  const protectedText=text.replace(/\b\d{2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{4}\b/g,match=>{
    const token=`__SAHALAT_DATE_${dates.length}__`;
    dates.push(match);
    return token;
  });
  const converted=protectedText.replace(/[0-9]/g,d=>'٠١٢٣٤٥٦٧٨٩'[Number(d)]);
  return converted.replace(/__SAHALAT_DATE_(\d+)__/g,(_,i)=>dates[Number(i)]??'');
}
