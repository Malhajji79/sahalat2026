...
function toArabicDigitsText(value){
  ...
  // Keep Sahalat display dates in Latin digits even when the UI language is Arabic.
  // Example: 05-Oct-2024 (never ٠٥-Oct-٢٠٢٤).
  // Split around complete display dates instead of using numbered placeholder tokens;
  // numbered tokens themselves were being converted to Arabic digits.
  const datePattern=/(\b\d{2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{4}\b)/g;
  return text.split(datePattern).map(part=>{
    if(/^\d{2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{4}$/.test(part)) return part;
    return part.replace(/[0-9]/g,d=>'٠١٢٣٤٥٦٧٨٩'[Number(d)]);
  }).join('');
}
...
function dateDisplay(value){
  ...
  const formatted=`${String(d).padStart(2,'0')}-${DATE_MONTHS[m-1]}-${String(y).padStart(4,'0')}`;
