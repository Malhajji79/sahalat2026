// The database is the only source for rate schedules and effective-dated shares.
function getLoanShares(date=todayISO()){
  const day=/^\d{4}-\d{2}-\d{2}$/.test(String(date||''))?String(date):todayISO();
  const policy=state.loanSharePolicies.filter(p=>p.effectiveFrom==='-infinity'||p.effectiveFrom<=day)
    .sort((a,b)=>a.effectiveFrom===b.effectiveFrom?0:a.effectiveFrom==='-infinity'?1:b.effectiveFrom==='-infinity'?-1:b.effectiveFrom.localeCompare(a.effectiveFrom))[0];
  if(!policy)throw Error('Share policy has not been loaded for this date');
...
function calcLoan(amount, months, generalDiscount, userDiscount, adminDiscount, sharePercentages){
  ...
  const shares=sharePercentages??getLoanShares();
  const userShare=shares.user/100;
  const ownerShare=shares.owner/100;
...
function canAccessAnalytics(){
  ...
  if(state.currentUser.role==='مدير مشروع') return true;
...
function analyticsEffectiveness(u, m=getUserAccountMetrics(u.username), maxLoans=Math.max(1,...analyticsUsers().map(x=>getUserAccountMetrics(x.username)?.totalCount||0))){ if(!m) return {score:0,collectionRate:0,closeRate:0,activity:0};
